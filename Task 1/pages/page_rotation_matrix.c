    ...
    if (prop_input_len != 0)
    {
        ...
        for (int i = 0; i < data->rotation_matrix->matrix_length; i++)
            for (int j = 0; j < data->rotation_matrix->matrix_length; j++)
            {
                ...
                string_matrix_set_cell(data->matrix, i, j, text, STRING_MATRIX_CELL_NORMAL);
            }
    } else
    {
...
void rotation_matrix_page_create(struct AppPage *page, GtkWidget *window)
{
    ...
    gtk_grid_attach(GTK_GRID(container), data->text_edit, 0, 1, 22, 1);
    ...
    gtk_grid_attach(GTK_GRID(container), text_selection_btn, 22, 1, 2, 1);
    ...
    gtk_grid_attach(GTK_GRID(container), data->summary_edit, 0, 2, 24, 1);
    ...
    GtkWidget *string_matrix_prev_step_btn = gtk_button_new_from_icon_name("pan-start");
    gtk_grid_attach(GTK_GRID(card_content), GTK_WIDGET(string_matrix_prev_step_btn), 0, 4, 2, 1);
    GtkWidget *string_matrix_step_label = gtk_label_new("Шаг 1 из 4");
    {
        ...
        gtk_label_set_attributes(GTK_LABEL(string_matrix_step_label), string_matrix_step_label_attrlist);
    }
    gtk_grid_attach(GTK_GRID(card_content), GTK_WIDGET(string_matrix_step_label), 2, 4, 20, 1);
    GtkWidget *string_matrix_next_step_btn = gtk_button_new_from_icon_name("pan-end");
    gtk_grid_attach(GTK_GRID(card_content), GTK_WIDGET(string_matrix_next_step_btn), 22, 4, 2, 1);
